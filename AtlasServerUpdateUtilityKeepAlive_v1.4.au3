...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtilityKeepAlive_v1.4.exe
#AutoIt3Wrapper_Outfile_x64=Builds\AtlasServerUpdateUtilityKeepAlive_v1.4(x64).exe
#AutoIt3Wrapper_Res_Comment=By Phoenix125
#AutoIt3Wrapper_Res_Description=AtlasServerUpdateUtilityKeepAlive_v1.4
#AutoIt3Wrapper_Res_Fileversion=1.4.0.0
#AutoIt3Wrapper_Res_ProductName=AtlasServerUpdateUtilityKeepAlive_v1.4
#AutoIt3Wrapper_Res_ProductVersion=v1.4.0
...
Local $iTrayLastUpdated = TrayCreateItem("Last Online Check:")
...
Do
	If Not $aPauseUtil Then
		...
		TrayItemSetText($iTrayLastUpdated, "Last Online Time: " & $tTime[1] & ":" & $tTime[2] & ":" & $tTime[3])
		If ((_DateDiff('s', $tLastAliveTime, _NowCalc())) >= $aKeepAliveTimout) And $aCloseUtilYN = "no" Then
			If $aCompiledYN = "no" Then
			...
			EndIf
			ProcessClose($aPID)
			SplashTextOn($aUtilName, "WARNING! " & $aProgramToRun & " closed unexpectedly. Restarting program.", 475, 140, -1, -1, $DLG_MOVEABLE, "")
			Sleep(5000)
...
Func ReadUini()
	...
	Global $aKeepAliveTimout = IniRead($aIniFile, $iHeaderMain, $iKeepAliveTimeout, $iniCheck)
	...
	EndIf
	If $iniCheck = $aKeepAliveTimout Then
		$aKeepAliveTimout = 180
		$iIniFail += 1
		$iIniError = $iIniError & "KeepAliveTimout, "
	EndIf
	If $aKeepAliveTimout < 90 Then $aKeepAliveTimout = 90
	If $aKeepAliveTimout > 600 Then $aKeepAliveTimout = 600
	If $iIniFail > 0 Then
		UpdateIni()
	EndIf
EndFunc   ;==>ReadUini
...
Func UpdateIni()
	...
	IniWrite($aIniFile, $iHeaderMain, $iKeepAliveTimeout, $aKeepAliveTimout)
EndFunc   ;==>UpdateIni
