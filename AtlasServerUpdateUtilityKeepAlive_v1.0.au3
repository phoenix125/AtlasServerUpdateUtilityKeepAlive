...
#AutoIt3Wrapper_Outfile=Builds\AtlasServerUpdateUtilityKeepAlive_v1.0.exe
#AutoIt3Wrapper_Outfile_x64=Builds\AtlasServerUpdateUtilityKeepAlive_v1.0(x64).exe
#AutoIt3Wrapper_Res_Comment=By Phoenix125
#AutoIt3Wrapper_Res_Description=AtlasServerUpdateUtilityKeepAlive_v1.0
#AutoIt3Wrapper_Res_Fileversion=1.0.0.0
#AutoIt3Wrapper_Res_ProductName=AtlasServerUpdateUtilityKeepAlive_v1.0
...
Global $aLogFile = $aFolderTemp & $aUtilName & ".log"
Global $aPID = 0
...
;~ Global $xPID = ProcessList()
;~ $xPID = ProcessList("AtlasServerUpdateUtility_v1.5.8(b5).exe")
;~ $aFileOpen = FileOpen(@ScriptDir & "\_start_AtlasServerUpdateUtility.bat")
;~ $aFileRead = FileRead($aFileOpen)
;~ FileClose($aFileOpen)
;~ $aProgramFull = _ArrayToString(_StringBetween($aFileRead, 'AtlasServerUpdateUtility" "', '"'))
;~ $aProgramEXE = _ArrayToString(_StringBetween($aProgramFull,
;~ Global $xPID = ProcessList()
;~ MsgBox(0,"Kim1", $aProgram[0])
;~ MsgBox(0,"Kim", _ArrayToString($xPID))
;~ _ArrayDisplay($xPID)
;~ Global $aPID = $xPID[1][1]
;~ Global $aPID = $aProgram

;~ If WinExists("[CLASS:AutoIt v3 GUI]", "AtlasServerUpdateUtility") Then
;~ 	Global $aPID = WinGetProcess("[CLASS:AutoIt v3 GUI]", "AtlasServerUpdateUtility")
;~ 	$aProgramToRun = @ScriptDir & "\AtlasServerUpdateUtilityKeepAlive.bat"
;~ 	MsgBox(0, $aUtilName, "Not compiled: PID[" & $aPID & "]", 5)
;~ Else
;~ 	Global $aPID = $aProgramToKeepAlive
;~ EndIf
	...
	If Not $aPauseUtil Then
		...
		If Not ProcessExists($aPID) And $aCloseUtilYN = "no" Then
			If WinExists("[CLASS:AutoIt v3 GUI]", "AtlasServerUpdateUtility") Then
				...
				$aProgramToRun = @ScriptDir & "\AtlasServerUpdateUtilityKeepAlive.bat"
				MsgBox(0, $aUtilName, "Not compiled: PID[" & $aPID & "]", 5)
			ElseIf WinExists($aProgramToRun) Then
				Global $aPID = $aProgramToKeepAlive
			Else
